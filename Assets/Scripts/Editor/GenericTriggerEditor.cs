...
public class GenericTriggerEditor : Editor {
	...
	protected virtual void OnEnable(){
		...
		string[] eventNames = Enum.GetNames(typeof(GenericTrigger.GenericTriggerType));
	...
	public override void OnInspectorGUI(){
		...
		for (int i = 0; i < m_DelegatesProperty.arraySize; ++i){
			...
			SerializedProperty callbacksColProperty = delegateProperty.FindPropertyRelative("callbackCollision");
			SerializedProperty callbacksTrigProperty = delegateProperty.FindPropertyRelative("callbackCollider");
			m_EventIDName.text = eventProperty.enumDisplayNames[eventProperty.enumValueIndex];

			{
				var id = (GenericTrigger.GenericTriggerType)eventProperty.intValue;
				if(GenericTrigger.IsCollision(id)){
					EditorGUILayout.PropertyField(callbacksColProperty, m_EventIDName);
				}else if(GenericTrigger.IsCollider(id)){
					EditorGUILayout.PropertyField(callbacksTrigProperty, m_EventIDName);
				}else{
	...
	private void OnAddNewSelected(GenericTrigger.GenericTriggerType type, UnityEvent newEvent){
	...
	private void OnAddNewSelected(GenericTrigger.GenericTriggerType type, GenericTrigger.UnityEventCollision newEvent){
		GenericTrigger gt = (GenericTrigger)target;
		gt.delegates.Add(new GenericTrigger.Entry(){
			eventID = type,
			callbackCollision = newEvent
		});
		EditorUtility.SetDirty(gt);
	}
